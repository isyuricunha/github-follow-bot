...
parser.add_argument("-a",
                    "--all",
                    help="Unfollow all your followings",
                    action="store_true")
parser.add_argument("-fo",
                    "--followers",
                    help="Only unfollow users who already follow you",
                    action="store_true")
...
if args.all:
    ...
    if args.non_followers:
        bot.getFollowers(following=True)
        nonFollowersFollowings = [
            v for v in bot.followings if v not in bot.usersToAction
        ]
...
filename = ("./logs/" + str(datetime.now().strftime("%m-%d-%Y__%H-%M")) +
            "__" + str(len(bot.usersToAction)) + "-unfollowed-users.json")
